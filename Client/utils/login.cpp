...
void Encrypt(int sock,unsigned char* digest, string uname,unsigned char* clear_buf_new, unsigned int clear_size_new,size_t aad1_new,size_t aad2_new,size_t aad3_new)
{
  ...
  // string clear_file_name="x";
   ...
   unsigned int enc_buffer_size = clear_size + block_size;
   ...
   //Create and initialise the context with used cipher, key and iv
    ...
    if(1 != EVP_EncryptInit(ctx, cipher, key, iv))
        handleErrors("EVP_EncrptInit failed");
...
unsigned char* final_cphr = (unsigned char*)malloc(final_length);
 ...
 cout<<"\n Final lenfth: "<<final_length<<endl;
 cout<<"\n cphr_size: "<<cphr_size<<"\ntag_len: "<<tag_len<<"\niv_len: "<<iv_len<<endl;
  // Copy iv
