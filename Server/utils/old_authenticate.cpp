...
int signcloud(int sock, unsigned char* client_Nonce, long int client_nonce_size) {
...
//    if(!cin) { cerr << "Error during input\n"; exit(1); }
   ...
   unsigned char clear_buf[clear_size+1];
   ...
   unsigned char* CDHpublic_buffer = new unsigned char [clear_size + client_nonce_size] ;
     CDHpublic_buffer[clear_size + client_nonce_size] = '\0';

   // Concatenate half key with nonce
  // memset(&CDHpublic_buffer, 0, (clear_size + client_nonce_size));
    cout<<"\n Here 1"<<endl;
     long int c_size = clear_size + client_nonce_size + 1 + 1; // +2 for null-terminators
   ...
   CDHpublic_buffer[clear_size] = '\0';
   memcpy(CDHpublic_buffer + clear_size + 1, client_Nonce, client_nonce_size);
   CDHpublic_buffer[c_size - 1] = '\0';

   //CDHpublic_buffer[clear_size+client_nonce_size] = '\0';
   
   //unsigned char* CDHpublic_buffer = new unsigned char[clear_size + client_nonce_size+1];
   //long int sizea = clear_size + client_nonce_size;
...
cout<<"\n Concatenated buf size: "<<c_size<<endl;
   ...
   // cout << "\n Concatenated new buf" << endl;
   // BIO_dump_fp(stdout, (const char *)new_buf, c_size);
...
long int client_nonce_size, len_user;

//Receiving client's nonce size and username size
read(sock, &client_nonce_size, sizeof(long int));
read(sock, &len_user, sizeof(long int));
cout<<"\n Check 1"<<endl;
//Now we receive the Message M1
char client_M1 [client_nonce_size+len_user + 1];
read(sock, client_M1, (client_nonce_size+len_user));
client_M1 [client_nonce_size+len_user] = '\0';

char username[len_user];
unsigned char client_Nonce[client_nonce_size +1];
memcpy(client_Nonce, &client_M1, client_nonce_size);
memcpy(username, reinterpret_cast<char*>(&client_M1) + client_nonce_size, len_user);
client_Nonce[client_nonce_size] = '\0';
